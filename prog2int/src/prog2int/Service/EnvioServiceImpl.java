...
public class EnvioServiceImpl implements GenericService<Envio> {
    ...
    private void validateEnvio(Envio e) {
        ...
        if (e.getTipo() == null) throw new IllegalArgumentException("El tipo de envío es obligatorio");
        if (e.getEstado() == null) throw new IllegalArgumentException("El estado del envío es obligatorio");

        // Dominio de enums (defensivo si vienen de UI)
        ensureEnum(e.getEmpresa(), Empresa.class, "empresa inválida");
        ensureEnum(e.getTipo(),    TipoEnvio.class, "tipo de envío inválido");
        ensureEnum(e.getEstado(),  EstadoEnvio.class, "estado de envío inválido");
